    ...
    return (
        <>
            ...
            <div className="mgr-card" style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end' }}>
                ...
                <button type="button" className="st-btn" style={{ width: 'auto', borderColor: '#f90', color: '#f90' }} onClick={logAdhoc}>LOG UNSCHEDULED ARRIVAL</button>
                <button type="button" className="st-btn" style={{ width: 'auto' }} onClick={addScheduled}>➕ ADD SCHEDULED DELIVERY</button>
            </div>
                                ...
                                <div>
                                    {pending ? (
                                        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end' }}>
                                            <button type="button" className="st-btn" style={{ width: 'auto', padding: '4px 12px', fontSize: '0.78em', borderColor: '#0f8', color: '#0f8' }} onClick={() => markTimeIn(e.exp_id)}>TIME IN</button>
                                            <button type="button" className="st-btn" style={{ width: 'auto', padding: '4px 12px', fontSize: '0.78em', borderColor: '#f33', color: '#f33' }} onClick={() => removeDelivery(e.exp_id)}>REMOVE</button>
                                        </div>
                                    ) : null}
                                    {onDock ? (
                                        <button type="button" className="st-btn" style={{ width: 'auto', padding: '4px 12px', fontSize: '0.78em', borderColor: '#8cf', color: '#8cf', marginTop: 8 }} onClick={() => markTimeOut(e.exp_id)}>TIME OUT</button>
                                    ) : null}
                                </div>
                            </div>
                        </div>
