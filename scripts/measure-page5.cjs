...
function ClampX($x) { return [math]::Max(0, [math]::Min($w - 1, [int]$x)) }
function ClampY($y) { return [math]::Max(0, [math]::Min($h - 1, [int]$y)) }
function PixelR($x, $y) { return $bmp.GetPixel((ClampX $x), (ClampY $y)).R }

Write-Output '--- SUPPORTING DOC HORIZ LINES ---'
for ($y = 250; $y -lt 1100; $y += 2) {
  $dark=0
  for ($x=60; $x -lt 1640; $x += 5) {
    if ((PixelR $x $y) -lt 85) { $dark++ }
  }
...
for ($y = 320; $y -lt 420; $y += 1) {
  for ($x = 80; $x -lt 400; $x += 1) {
    if ((PixelR $x $y) -gt 60) { continue }
    # cheap local density
    $d=0
    for ($dy=0; $dy -lt 14; $dy++) {
      for ($dx=0; $dx -lt 14; $dx++) {
        if ((PixelR ($x+$dx) ($y+$dy)) -lt 100) { $d++ }
      }
    }
